...
export const Main = () => {
  ...
  const [loadingPowerIndex, setLoadingPowerIndex] = useState<number | null>(null);
  ...
  return (
    <div className="relative grow w-full bg-[linear-gradient(180deg,rgba(0,0,0,0.32)_0%,rgba(0,0,0,0.12)_100%)] px-16 py-12" onClick={(e) => { e.stopPropagation(); setNextNumberModal(false); }}>
      {gameOverModal && 
      ...
      {nextNumberModal && 
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 p-6" onClick={(e) => e.stopPropagation()}>
          <GameNextNumber number={game.next_number} setModal={setNextNumberModal} />
        </div>
      }
      <div className="h-full max-w-[624px] mx-auto flex flex-col gap-4 justify-center">
        <div className="flex flex-col gap-12">
          <GameHeader game={game} tournament={tournament} onApplyPower={handleApplyPower} loadingPowerIndex={loadingPowerIndex} loadingSlotIndex={loadingSlotIndex} />
          <GameGrid game={game} onSetSlot={handleSetSlot} loadingSlotIndex={loadingSlotIndex} highlights={game.over ?game.closests() : []} alloweds={game.alloweds()} />
...
export const GameHeader = ({ game, tournament, onApplyPower, loadingPowerIndex, loadingSlotIndex }: { game: GameModel, tournament: TournamentModel, onApplyPower: (powerIndex: number) => Promise<void>, loadingPowerIndex: number | null, loadingSlotIndex: number | null }) => {
  return (
    <div className="w-full flex justify-between">
      <GameNumber number={game.number} over={game.over} isLoading={loadingPowerIndex !== null || loadingSlotIndex !== null} />
      <PowerUps powers={tournament.powers} availables={game.powers} count={game.adjacent()} onApplyPower={onApplyPower} loadingPowerIndex={loadingPowerIndex} />
    </div>
  )
}

export const GameNumber = ({ number, over, isLoading }: { number: number, over: boolean, isLoading: boolean }) => {
  return (
    <div className="flex flex-col gap-2">
      <span className="text-purple-300 tracking-wider text-lg/6">Your number is...</span>
      <div className={cn("text-[136px]/[100px] font-normal", over ? "text-red-100" : "text-white-100")} style={{ textShadow: '4px 4px 0px rgba(28, 3, 101, 1)' }}>
        <SlotCounter
...
export const GameNextNumber = ({ number, setModal }: { number: number, setModal: (modal: boolean) => void }) => {
  const power = new Power(PowerType.Foresight);
  return (
    <div className="h-64 w-[288px] bg-black-300 border-[2px] border-black-300 backdrop-blur-[4px] rounded-lg p-6 flex flex-col items-center gap-6" style={{ boxShadow: '0px 4px 4px 0px rgba(0, 0, 0, 0.25)' }}>
      <img src={power.icon(undefined)} alt={power.name()} className="size-16" />
      <p className="text-white-100 text-[64px]/[44px]" style={{ textShadow: '4px 4px 0px rgba(28, 3, 101, 1)' }}>{number}</p>
      <Button variant="secondary" className="w-full px-6 py-1" onClick={() => setModal(false)}>
        <p className="text-[28px]/[19px] tracking-wider translate-y-0.5" style={{ textShadow: '2px 2px 0px rgba(0, 0, 0, 0.24)' }}>Close</p>
      </Button>
    </div>
  )
}

export const PowerUps = ({ powers, availables, count, onApplyPower, loadingPowerIndex }: { powers: Power[], availables: Power[], count: number, onApplyPower: (powerIndex: number) => Promise<void>, loadingPowerIndex: number | null }) => {
  return (
      ...
      <div className="flex gap-3">
        {powers.map((power) => (
          <PowerUp 
            ...
            isDisabled={loadingPowerIndex !== null && loadingPowerIndex !== power.index()}
          />
        ))}
      </div>
    </div>
...
export const PowerUp = ({ power, available, count, onApplyPower, isLoading, isDisabled }: { power: Power, available: boolean, count: number, onApplyPower: (powerIndex: number) => Promise<void>, isLoading: boolean, isDisabled: boolean }) => {
    ...
    <div className="flex flex-col gap-2 items-center justify-between text-purple-300">
      <TooltipProvider delayDuration={150}>
        <Tooltip>
          <TooltipTrigger>
            <Button 
              disabled={!available || power.isLocked(count) || isDisabled || isLoading} 
              variant="muted" 
          ...
          <TooltipContent className="max-w-[288px] bg-black-300 border-[2px] border-black-300 rounded-lg p-6 flex flex-col gap-3" style={{ boxShadow: '0px 4px 4px 0px rgba(0, 0, 0, 0.25)' }}>
            <h2 className="text-white-100 tracking-wider text-[22px]/[15px] translate-y-0.5">{power.name()}</h2>
            <p className="text-white-100 font-ppneuebit text-2xl/5">{power.description()}</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
...
const GameGrid = ({ game, onSetSlot, loadingSlotIndex, highlights, alloweds }: { game: GameModel, onSetSlot: (index: number) => Promise<void>, loadingSlotIndex: number | null, highlights: number[], alloweds: number[] }) => {
  return (
    <div className="grid grid-flow-col grid-rows-5 gap-x-16 gap-y-4 font-ppneuebit">
      {game.slots.map((slot, index) => (
        <GameSlot 
          ...
          isDisabled={game.over || (loadingSlotIndex !== null && loadingSlotIndex !== index) || !alloweds.includes(index)}
      ...
      {slot ? 
        ...
        >
          {isLoading ? (
            ...
            <p className="text-2xl" style={{ textShadow: !isDisabled ? '2px 2px 0px rgba(0, 0, 0, 0.85)' : undefined }}>Set</p>
          )}
        </Button>
      }
    </div>
...
export const GameOver = ({ game }: { game: GameModel }) => {
  return (
    <div className="w-full h-full select-none" onClick={(e) => e.stopPropagation()}>
      <div className="relative w-full h-full rounded-2xl bg-black-300 border-[2px] border-black-300 backdrop-blur-[4px] p-6 flex flex-col gap-12 justify-center items-center">
        <h1 className="text-[136px]/[92px]" style={{ textShadow: '5px 5px 0px rgba(26, 5, 87, 1)' }}>Game Over!</h1>
...
export const GameOverScore = ({ score }: { score: number }) => {
  return (
    <div className="grow flex flex-col gap-3 justify-between px-5 py-4 bg-white-900 border border-white-900 rounded-xl">
      ...
      <p className="text-[28px]/[19px] tracking-wider" style={{ textShadow: '2px 2px 0px rgba(0, 0, 0, 0.25)' }}>{score}</p>
    </div>
  )
}
...
export const GameOverEarning = ({ earning }: { earning: number }) => {
  return (
    <div className="grow flex flex-col gap-3 justify-between px-5 py-4 bg-white-900 border border-white-900 rounded-xl">
      ...
      <p className="text-[28px]/[19px] tracking-wider" style={{ textShadow: '2px 2px 0px rgba(0, 0, 0, 0.25)' }}>{`${earning.toLocaleString()} NUMS`}</p>
    </div>
  )
}
  ...
  return (
    <Link to="/">
      <Button variant="secondary" className="gap-1 h-[56px] w-[146px]">
        ...
        <p className="text-[28px]/[19px] tracking-wider translate-y-0.5 px-1" style={{ textShadow: '2px 2px 0px rgba(0, 0, 0, 0.25)' }}>Home</p>
      </Button>
    </Link>
  )
}
