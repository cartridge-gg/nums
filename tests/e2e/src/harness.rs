...
use crate::constants::{
    APPCHAIN_CHAIN_ID_STR, DEV_ACCOUNT_0_ADDRESS, DEV_ACCOUNT_0_PRIVKEY, DEV_ACCOUNT_0_PUBKEY,
    DEV_ACCOUNT_1_PRIVKEY, SETTLEMENT_CHAIN_ID_STR,
};
...
use crate::rollup::{
    deploy_account_on_settlement, init_rollup, write_appchain_profile_toml, GenesisAccount,
};
...
impl TestEnv {
    pub async fn start() -> Result<Self> {
        ...
        // 2b. Deploy the rollup genesis account ALSO on the settlement
        //     chain via a `DeployAccount` v3 transaction. Once that's
        //     mined, the appchain rollup genesis account (which lives
        //     at `rollup.appchain_account.address` on the appchain by
        //     virtue of the auto-generated chain spec) ALSO lives at
        //     the same address on settlement, so the e2e test can sign
        //     `Play.claim` on settlement and `Play.set` on the appchain
        //     with the same key.
        //
        //     Why not patch the appchain genesis.json to predeploy
        //     `DEV_ACCOUNT_0` instead: rollup-mode Katana derives the
        //     deploy address from `get_contract_address(salt, class_hash,
        //     [pubkey], 0)` and IGNORES the JSON map key. `--dev` mode
        //     Katana derives the address using the `frozen_address_class_hash`
        //     (`0x07dc7899…`), so the same `(salt, pubkey)` produces
        //     different addresses on the two modes. The rollup genesis
        //     account, by contrast, was generated by `katana init rollup`
        //     under the rollup formula, so re-deploying it on settlement
        //     under the same formula lands at exactly the same address.
        deploy_account_on_settlement(
            &Self::provider(&settlement.rpc_url())?,
            settlement_chain_id,
            &rollup.appchain_account,
        )
        .await
        .context("deploy rollup genesis account on settlement")?;
    ...
    /// DEV_ACCOUNT_0 signed against the APPCHAIN. Required for the
    /// reverse direction — the test player must be signable on both
    /// chains, so `TestEnv::start` patches `genesis.json` to add
    /// DEV_ACCOUNT_0 to the appchain's pre-deployed accounts. After
    /// that patch, this constructor produces a working SingleOwnerAccount
    /// for sending `Play.set` / `Play.claim` from the player.
    pub fn dev_account_appchain(
        &self,
    ) -> Result<SingleOwnerAccount<JsonRpcClient<HttpTransport>, LocalWallet>> {
        Ok(build_account(
            Self::provider(&self.appchain.rpc_url())?,
            self.appchain_chain_id,
            DEV_ACCOUNT_0_ADDRESS,
            DEV_ACCOUNT_0_PRIVKEY,
        ))
    }
